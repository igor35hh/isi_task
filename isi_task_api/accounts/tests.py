...
class CustomUserTest(TestCase):
    ...
    def test_model_auth_user(self):
        ...
        auth_test_admin = AccountAuthBackend().authenticate(
            None, test_admin.username, 'admin@1234')

        auth_test_driver = AccountAuthBackend().authenticate(
            None, test_driver.username, 'admin@1234')
    ...
    def test_get_view_single_user(self):
        ...
        view = CustomUserViewSet.as_view(
            {'get': 'retrieve'})(response.wsgi_request, pk=self.test_admin.pk)
        ...
        view = CustomUserViewSet.as_view(
            {'get': 'retrieve'})(response.wsgi_request, pk=self.test_driver.pk)

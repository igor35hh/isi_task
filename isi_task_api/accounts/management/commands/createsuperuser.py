...
class Command(createsuperuser.Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.user_type_field = self.UserModel._meta.get_field(self.UserModel.USERTYPE_FIELD)
        ...
        if not options['interactive']:
            ...
            try:
                ...
                while type_of_user is None:
                    ...
                    if default_usertype:
                        input_msg += " (input '%s' or leave blank to use '%s')" % (USERTYPE_ADMIN, default_usertype)
                    ...
                    type_of_user = self.get_input_data(self.user_type_field, input_msg, default_usertype)
                    if not type_of_user:
                        continue

                if not type_of_user:
                    raise CommandError('%s cannot be blank.' % capfirst(verbose_type_of_user))
